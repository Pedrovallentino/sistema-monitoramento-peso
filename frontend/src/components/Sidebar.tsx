...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <div className={clsx(
        "fixed inset-y-0 left-0 z-30 w-72 bg-white border-r border-gray-100 transform transition-transform duration-300 ease-in-out shadow-xl flex flex-col",
        isOpen ? "translate-x-0" : "-translate-x-full"
      )}>
        {/* Logo Area */}
        <div className="p-8 flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-xl flex items-center justify-center text-white shadow-emerald-200 shadow-lg">
            <Flame size={20} fill="currentColor" />
          </div>
          <div>
            <span className="text-xl font-bold text-gray-800 block leading-none">SmartGás</span>
        ...
        <nav className="flex-1 px-4 space-y-2 py-4 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon size={20} className={clsx("transition-colors", isActive ? "text-emerald-600" : "text-gray-400 group-hover:text-gray-600")} />
                {item.label}
                {isActive && (
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
            ...
            <div className="flex justify-between items-center">
               <p className="text-sm font-bold text-gray-700">v2.1.0</p>
